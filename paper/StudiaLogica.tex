...
We say a logic $\mc{L}$ has Craig interpolation property if for any formula $A \to C$ provable in $\mc{L}$, there exists a formula $B$ such that $A \to B$ and $B \to C$ are provable in $\mc{L}$, satisfying the variable condition: $\vars{B} \subseteq \vars{A} \cap \vars{C}$ ($\vars{A}$ is the set of atomic formulae appearing in $A$ and $\to$ is the implication connective in $\mc{L}$).
Craig interpolation has been employed to prove model-theoretical results, including Beth's definability theorem \cite{Beth1953}, and has also found applications in model checking \cite{Henzinger2004}.
...
Its non-associative variant, which has been extensively studied, also disallows associativity. Another significant example is linear logic, introduced by Jean-Yves Girard \cite{girard:linear:87}, where weakening and contraction are disallowed but can be recovered for specific formulas through modalities.
Substructural logics have proven useful in many areas, from analyzing natural language syntax computationally to creating programming languages sensitive to resource management.

Craig interpolation for substructural logics has been studied extensively, both from algebraic and proof-theoretic perspectives. 

For substructural logics that lack a cut-free sequent calculus, such as arbitrary extensions of the full Lambek calculus with exchange ($\FL_{\textbf{e}}$), Craig interpolation is established using algebraic methods.
...
In the case of \FL, for instance, the approach begins by establishing a stronger form of interpolation which we call Maehara interpolation property \cite{ono:proof:nonclassical:1998}. 
The property states:
\begin{itemize}
  \item[\ ] Given $f : \Gamma \vdash C$ and any partition $\langle \Gamma_0, \Gamma_1, \Gamma_2 \rangle$ of $\Gamma$, there exist a formula $D$ and two derivations $g : \Gamma_1 \vdash D$ and $h : \Gamma_0, D, \Gamma_1 \vdash C$, and $\vars{D} \subseteq \vars{\Gamma_0} \cap \vars{\Gamma_0, \Gamma_1, C}$
\end{itemize}
It is a stronger form of the so-called deductive interpolation property. A logic $\mc{L}$ is said to have the deductive interpolation property if, for any formulae $A$ and $C$, whenever $A \vd C$ (where $\vd$ is the consequence relation of $\mc{L}$), then there exists a formula $B$ such that $A \vd B$ and $B \vd C$ while also satisfying the variable condition. Furthermore, if the sequent calculus of $\mc{L}$ admits the invertibility of implication right rules, Craig interpolation follows as a consequence of the deductive interpolation property.

A variant of Maehara interpolation is necessary for proving Craig interpolation in logics that lack multiplicative (additive) conjunction.
This includes sequent calculi such as the product-free Lambek calculus \cite{Pentus1997} and the implicational fragment of intuitionistic logic \cite{Kanazawa2006}. 
The property described in \cite{Pentus1997} is particularly relevant. We have modified it to align with our discussion.
\begin{itemize}
  \item[\ ]  Given $f: S \mid \Gamma \vd C$ and any partition $\langle \GG_0,\GG_1, \GG_2 \rangle$ of $\GG$, there exist a partition of $\langle \GD_1, \dots, \GD_n \rangle$ of $\GG_1$ and a list of interpolant formulae $D_1, \dots, D_n$ such that
  \begin{itemize}
    \item[--] $g: \GG_0, D_1, \dots, D_n, \GG_2 \vd C$,
    \item[--] $h_i : \GD_i \vd D_i$ for $i \in [1\dots n]$,
    \item[--] $\gs{D_i} \leq \gs{\GD_i}$ for $i \in [1 \dots n]$ and $\gs{D_1, \dots, D_n} \leq \gs{S, \GG_0, \GG_2, C}$ for every $X$ ($\gs{A}$ counts the number of occurrences of the atomic formula $X$ in $A$.).
  \end{itemize}
\end{itemize}
Instead of finding just one formula as an interpolant, we look for a list of formulas.
This adjustment allows us to overcome the difficulty caused by the absence of conjunction.

In this paper, we aim at proving Craig interpolation for the \emph{semi-substructural} logic \SkNMILL~in \cite{UVW:protsn}.
A logic is semi-substructural if it is an intermediate logic in between (certain fragments of) non-associative and associative intuitionistic linear logic (or Lambek calculus).
...
Semi-substructural logics are motivated by \emph{left skew monoidal categories} \cite{szlachanyi:skew-monoidal:2012}.
These categories are a weaker variant of MacLane's monoidal categories.
In left skew monoidal categories, the structural morphisms for associativity and unitality are not required to be bidirectional. Instead, they are natural transformations with a specific orientation.
Therefore, they can be seen as \emph{semi-associative} and \emph{semi-unital} variants of monoidal categories. 

Different variants of left skew monoidal categories have led to the development of corresponding semi-substructural logics.
...
Each of these logics admits a cut-free sequent calculus and a subcalculus inspired by Jean-Marc Andreoli's focusing method \cite{andreoli:logic:1992}.
The focusing method helps solve the coherence problems for the corresponding variants of left skew monoidal categories.

To prove Craig interpolation for \SkNMILL, we need to modify the statement of Maehara interpolation.
This modification is required due to the issue similar to those encountered in the product-free Lambek calculus \cite{Pentus1997} and the implicational fragment of intuitionistic logic
...
%   , where $s(S) = I$ if $S = {-}$ or $s(S) = B$ if $S = B$.
  ...
  \end{itemize}
\end{restatable*}
In \SkNMILL, $\mf{scut}$-interpolation resembles Maehara interpolation, whereas $\mf{ccut}$-interpolation is similar to the interpolation discussed by Pentus and Kanazawa.

Motivated by the categorical interpretation of \SkNMILL, we investigate how proofs and their equivalences behave under the interpolation procedure. 
We introduce an equivalence relation on derivations ($\circeq$) that captures eta and permutative conversions, and is both sound and complete with respect to the categorical semantics.
Specifically, we focus on proof-relevant interpolation in the sense of {\v{C}}ubri{\'c} \cite{Cubric1994} and as recently discussed by Saurin \cite{Saurin2024}. 
In our framework, this entails that $\mf{scut}$/$\mf{ccut}$-interpolation serve as a left inverse of the admissible rules $\mf{scut}$ and $\mf{ccut}$, respectively.
The statements are
...
The proofs of Maehara interpolation and Craig interpolation are formalized in the proof assistant Agda.
