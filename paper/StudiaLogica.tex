...
\begin{displaymath}
    ...
    \end{displaymath}
% By soundness and completeness, similar to the result in \cite{uustalu:sequent:2021} for skew monoidal categories, we mean that \SkNMILL~is syntactically equivalent to the axiomatic characterization of the free left skew monoidal closed category.
...
% The calculus \SkNMILL, being an equivalent presentation of a skew version of \NL,\  provides an effective procedure to determine formulae derivability in \LSkNL.
% In other words, for any formula $A$, $ \vdL A$ if and only if ${-} \mid \quad \vdG A$.
% Proof search in \SkNMILL~always terminates, so for any $A$, either there is a proof in \SkNMILL~or the proof search terminates without finding any proof.
...
% This unsatisfying result motivates us to consider another calculus equivalent to \SkNMILL.
...
% % \end{proof}
% We have confirmed that \LSkT~enjoys interpolation, so the next step is to check whether it is equivalent to \SkNMILL.
...
% \end{displaymath}
% \begin{lemma}
%   The rule $\tl$ is invertible in \SkNMILL, i.e. this rule
%   \begin{displaymath}
%     \infer[\tl^{-1}]{A \mid B , \GG \vdG C}{
%       \deduce{A \ot B \mid \GG \vdG C}{f}
%     }
...
% \end{proof}
% \begin{proof}
%   Notice that $\mf{scut}$ is admissible in \SkNMILL, so we only have to check whether $T[A]^* \mid \quad \vdG T[B]^*$ is derivable from $g$.
...
% \end{displaymath}
% \end{defn}
% With the above lemmata, definition, and functions $s(S)$ that maps a stoup to a formula and $T^*$ that transforms trees into formulae, we can state and prove the equivalence between \SkNMILL~and \LSkT.
% \begin{theorem}\label{thm:equiv:LSkGLSKT}
% The calculi \SkNMILL~and \LSkT~are equivalent, meaning that the two statements below are true:
%   \begin{itemize}
...
% \end{proof}
% By the equivalence of \SkNMILL~and \LSkT, we can prove $\mf{scut}$ interpolation.
% % We define $\GT$-sequents and $\GT$-derivations of \SkNMILL~analogously to the \LSkT~case.
...
% Although we can prove $\mf{scut}$ interpolation for \SkNMILL~via \LSkT, we are still not able to prove it directly in \SkNMILL.
...
% The reason why \LSkT~enjoys interpolation but \SkNMILL~does not is that although the antecedent looks flat in \SkNMILL~, they are trees associating to the left.
...
% Intuitively, we can think of \SkNMILL~as a variation of \LSkT~in which, from the top-down perspective, antecedents are always normalized after each rule application.
...
% Therefore, the flat antecedents in \SkNMILL~lead to impossible cases when proving $\mf{ccut}$ interpolation.
...
% However, in \SkNMILL, this causes a problem because $\ot\mf{C}$ is inadmissible.
...
% Consequently, the flat context conceals the left-associative structure of antecedents in \SkNMILL, leading to impossible cases when proving interpolation.
...
% The sequent calculus for left skew monoidal categories (\SkNMILLm) has sequents of the form $S \mid \GG \vdG A$, but formulae are generated by the grammar $A,B::= X \mid \I \mid A \ot B$ and rules are those in (\ref{eq:seqcalc:skmc:Gir}) without $\lleft$ and $\lright$ \cite{uustalu:sequent:2021}.
% We call its equivalent sequent calculus with trees as antecedents \LSkTm.
% As mentioned in Section \ref{sec:intro}, \SkNMILLm~has a special form of Craig interpolation, i.e. it has $\mf{scut}$ Craig interpolation.
% Moreover, we can prove that \LSkTm~has Craig interpolation as well because the counterexample does not hold in \LSkTm.
% In general, \SkNMILLm~lacks both Craig and weak $\mf{ccut}$ interpolation due to the inadmissibility of $\ot\mf{C}$.
...
% This paper addresses two limitations of the sequent calculus \SkNMILL~in the previous work on proof theoretical analysis of skew monoidal closed category \cite{UVW:protsn}, i.e. lacking of Craig interpolation and being unable to modularly characterize skew bi-closed categories.
...
% \LSkT~is equivalent to \SkNMILL~and enjoys Craig interpolation, and by the equivalence of \LSkT~and \SkNMILL, we prove $\mf{scut}$ interpolation for \SkNMILL~via \LSkT. 
