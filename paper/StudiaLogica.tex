...
\begin{example}
  ...
  \\
  Following the procedure in the proof of Theorem \ref{genIntrp}, we need to apply the \cMMIP~procedure to the derivation $\pass (\lleft ((\pass \ \ax), \tr (\ax , \pass \ \ax))) : {-} \mid X \lolli Y , X, W \vd Y \ot W$ (witnessing the left premise of $\lleft$) with the partition $\langle [X \lolli Y] , [X, W] , [\ ] \rangle$.
  This produces
  \begin{itemize}
  ...
  \begin{displaymath}
    ...
    \end{array}
  \end{displaymath}
  This is, however, different from case of the associative Lambek calculus (with $\ot$).
  Consider the similar derivation of (\ref{example:interpolant}) in the associative Lambek calculus:
  \begin{displaymath}
    \begin{array}{c}
    ...
    \end{array}
  \end{displaymath}
  By the interpolation procedure defined in \cite{moot:categorial:2012}, the interpolant formula is instead ${Z \sls (X \ot W)}$, which is constructable due to the presence of $\ot$.
\end{example}
