...
For \SkNMILL, we should prove the following two lemmata of general interpolation.
These two lemmata are proved mutually by induction on derivations. 
We separate the proof to two parts for readability.
\begin{lemma}\label{lem:scut:intrp}
~
...
\begin{theorem}\label{thm:craig:intrp}
  For any formulae $A$ and $C$, if $A \lolli C$ is provable, then there exists a formula $B$ such that both $A \lolli B$ and $B \lolli C$ are provable, and $\gs{B} \leq \gs{A} $ and $ \gs{B} \leq \gs{C}$ for every $X$.
\end{theorem}
\begin{proof}
...
\end{proof}
\begin{remark}
  We state Craig interpolation with multiplicity condition instead of variable condition.
  However, the arguments above also applies to the proof of Craig interpolation with variable condition.
\end{remark}
The procedure in the proofs of Lemmata \ref{lem:scut:intrp} and \ref{lem:ccut:intrp} also provides a proof-relevant content of interpolation.
% In particular, we can prove the following two theorems.
\begin{theorem}
  If $g : S \mid \GG_0 \vd D$ and $h : D \mid \GG_1 \vd C$ are derivations obtained by applying Lemma \ref{lem:scut:intrp} on a derivation $f: S \mid \GG \vd C$ with the partition $\langle \GG_0 , \GG_1 \rangle$, then $\mf{scut}(g, h) \circeq f$.
\end{theorem}
\begin{definition}
  Suppose that there are $f : S \mid \GG_0 , D_1 , \dots, D_n , \GG_2 \vd C$, and a list of derivations $\phi$ where for any $h_i \in \phi$, $\mf{ccut}(h_i , g)$ is a derivation. 
  We define $\mf{ccut}^* (\phi , f)$ (iterated version of $\mf{ccut}$) recursively:
  \begin{itemize}
...
\end{definition}
\begin{theorem}
  If $g : S \mid \GG_0 , D_1 , \dots, D_n, \GG_2 \vd C$ and $h_i : {-} \mid \GD_i \vd D_i$ for $i \in [1 \dots n]$ are derivations obtained by applying Lemma \ref{lem:ccut:intrp} on a derivation $f: S \mid \GG \vd C$ with the partition $\langle \GG_0 , \GG_1 , \GG_2 \rangle$, then $\mf{ccut}^*([h_i] , g) \circeq f$.
\end{theorem}
