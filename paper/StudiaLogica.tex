...
Let us illustrate the interpolation procedure on a simple example.
 We compute the stoup Maehara interpolant of the end-sequent in the derivation 
  ...
  \\
  Following the procedure in the proof of Theorem \ref{genIntrp}, we need to apply the \cMMIP~procedureto the derivation $\pass (\lleft ((\pass \ \ax), \ax)) : {-} \mid X \lolli Y , X \vd Y$ (witnessing the left premise of $\lleft$) with the partition $\langle [X \lolli Y] , [X] , [\ ] \rangle$.
This produces
  \begin{itemize}
    \item[--] a partition $\langle [X] \rangle$ of $[X]$,
    \item[--] a singleton list of interoplant formulae $[X]$, and
  ...
  Then we obtain the desired interpolant formula $X \lolli Z$ and the desired derivations 
  \begin{displaymath}
    \begin{array}{lc}
      \begin{array}{c}
        g
