...
Maehara interpolation also holds for the commutative ($\FL_{\textbf{e}}$) and non-associative variants of \FL, respectively.
For the former, $\GG$ is partitioned as two multisets which ignores the order. For the latter, \MIP\ is reformulated where antecedents are trees of formulae instead of lists.
...
We restrict the application of introduction rules in an appropriate way to allow only one of the directions of associativity and unitality, i.e. only $(A \ot B)\ot C \mid \quad \vd A \ot (B \ot C)$, $\I \ot A \mid \quad \vd A$, and $A \mid \quad \vd A \ot \I$ are provable in \SkNMILL, while their inverses $A \ot (B \ot C) \mid \quad \vd (A \ot B) \ot C$, $A \mid \quad \vd \I \ot A$, and $A \ot \I \mid \quad \vd A$ are not.
