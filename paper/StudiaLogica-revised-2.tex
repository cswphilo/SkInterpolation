...
\FL~without additive connectives enjoys a stronger variant of Maehara interpolation where the variable condition is replaced by a variable \emph{multiplicity} condition \cite{moot:categorial:2012}.
...
In his PhD thesis \cite{roorda1991}, Roorda considered a stronger variable multiplicity condition, characterizing the \emph{positive and negative} occurrences of atomic formulae in interpolants. Let $\mathsf{At}_{+}(A)$ be the multiset of positive occurrences of atomic formulae in $A$, and let $\mathsf{At}_{-}(A)$ be the multiset of negative occurrences of atomic formulae in $A$. These notions can be straightforwardly extended to stoups and contexts. We can prove that the \sMIP~procedure satisfies the following stronger variable condition: given $p \in \{ +,-\}$, there is an injective function from $\mathsf{At}_p(D)$ to $\mathsf{At}_p(S,\Gamma_0)$, and an injective function from $\mathsf{At}_p(D)$ to the multiset union of $\mathsf{At}_{\neg p}(\Gamma_1)$ and $\mathsf{At}_p(C)$. Here $\neg p$ denotes the opposite polarity of $p$, i.e. $\neg + = -$ and $\neg - = +$. We can also prove that the \cMMIP~procedure satisfies the following stronger variable condition: given $p \in \{ +,-\}$, there is an injective function from $\mathsf{At}_p(D_1,\dots,D_n)$ to $\mathsf{At}_p(\Gamma_1)$ and  and an injective function from $\mathsf{At}_p(D_1,\dots,D_n)$ to the multiset union of $\mathsf{At}_{\neg p}(S,\Gamma_0,\Gamma_2)$ and $\mathsf{At}_p(C)$.
We refer to our Agda formalization for more details on how to establish these stronger variable conditions.
...
For \sMIP, the interpolant tree always exists. For any derivation $f : S \mid \GG_0 , \GG_1 \vd C$ and any partition $\langle \GG_0, \GG_1 \rangle$, $((S , \GG_0) , \GG_1)$ is a tree associated to the left where the interpolant tree $(S , \GG_0)$ has appeared.  For example, consider the derivation above and the partition $\langle [Y] , [Z]\rangle$. The interpolant tree $(X, Y)$ has appeared in the endsequent.
...
%Our proof of Craig interpolation clearly reflects this fact:
...
\bibliography{StudiaLogica-revised-2}
