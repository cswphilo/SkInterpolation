...
\begin{proof}[Proof of \sMIP]
  ...
  \begin{itemize}
    ...
    The variable multiplicity condition is satisfied because $\vars{\unit} = \emptyset$.
...
\begin{proof}[Proof of \cMMIP]
  ...
  \begin{itemize}
    ...
    The variable multiplicity condition is satisfied because $\gs{D_i} \leq \gs{ \GG'_0 , \GG_2 , B} \leq \gs{S, \GL, \GG'_0 , \GG_2 , A \ot B}$ for $i \in [1, \dots, n]$ and any $X$.
...
%We start by recalling some basic notions and notation.
...
\noindent\textbf{Sequents.} %The entailment relation $\_{\mid}\_{\vdash}\_ : \Stp \to \Cxt \to \Fma \to \Set$ is defined as an inductive type family.
Given a stoup $S : \Stp$, a context $\Gamma : \Cxt$ and a formula $C : \Fma$, the type $S \mid \Gamma \vdash C$ consists of all the possible proofs of the corrisponding sequent.
...
\begin{array}{rl}
  ...
  \;\; \field \\
  \;\; Ds &: \List ~\Fma \\
  \;\; g &: S \mid \GG_0 \append Ds \append \GG_2 \vdash D \\
  \;\; hs &: \GG_1 \vdash^* Ds
\end{array}
\]
Therefore elements of type $\sMIP ~S ~\GG_0 ~\GG_1 ~C$ are triples consisting of a formula $D$ and two derivations $g$ and $h$.
Elements of $\cMMIP ~S ~\GG_0 ~\GG_1 ~\GG_2 ~C$ are also triples, but consisting of a list of formulae $Ds$ (the interpolant formulae $D_1,\dots,D_n$ in Theorem \ref{genIntrp}), a derivation $g$ and a sequence of derivations $hs$ (the derivations $h_1,\dots,h_n$ in Theorem \ref{genIntrp}).
...
This implies that in our implementation of \cMMIP~ in Agda, the partition of $\GG_1$ and the derivations $h_i$ in the statement of \cMMIP~ in Theorem \ref{genIntrp} are both collected in the last field $hs : \GG_1 \vdash^* Ds$.
...
For each formula we define a function collecting the atomic formulae appearing in it, defined by recusion on the input formula:
\[
\begin{array}{lcl}
...
In the types below, the formula $\sMIP.D~(\mathsf{smip}~f~eq)$ is the interpolant computed by the stoup Maehara interpolation procedure on input $f$. Simialrly, $\sMIP.Ds~(\mathsf{cmmip}~f~eq)$ is the sequence of interpolant formulae produced by the context
...
This approach could be seen as a step towards the universal proof theory for semi-substructural logics in the sense of \cite{Tabatabai2025}.
