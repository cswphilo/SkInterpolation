...
\title[Craig Interpolation for a Semi-{S}ubstructural Logic]{Craig Interpolation for a Semi-{S}ubstructural Logic}
...
%which has been extensively studied \cite{moot:categorial:2012},
...
For substructural logics that admit a cut-free sequent calculus, Craig interpolation is typically proven by adapting Maehara's method \cite{maehara1961}, which originally aimed to prove interpolation for $\mathtt{LK}$, a sequent calculus for classical logic.
...
In the latter case, $\GG$ is an unordered list of formulae, i.e. a finite multiset, and it is partitioned as a pair of multisets instead of a triple of lists.
...
%For the latter, \MIP\ is reformulated where antecedents are trees of formulae instead of lists.
...
\end{description}
The $\gs{A}$ above is a function to count the number of occurrences of the atomic formula $X$ in $A$, which can be extended to count the number of occurrences of the atomic formula $X$ in a list of formulae. 
...
\begin{description}
  ...
  \begin{itemize}
    ...
    \item[--] derivations $h_i : \GD_i \vd D_i$ for all $i \in [1, \dots, n]$, such that
    \item[--] $\gs{D_i} \leq \gs{\GD_i} $ and $\gs{D_i} \leq \gs{\GG_0, \GG_2, C}$ for $i \in [1, \dots, n]$ for all atomic formulae $X$.
  \end{itemize}
\end{description}
...
We restrict the application of introduction rules in an appropriate way to allow only one of the directions of associativity and unitality, i.e. only the sequents $(A \ot B)\ot C \mid \quad \vd A \ot (B \ot C)$, $\I \ot A \mid \quad \vd A$, and $A \mid \quad \vd A \ot \I$ are provable in \SkNMILL, while their inverses $A \ot (B \ot C) \mid \quad \vd (A \ot B) \ot C$, $A \mid \quad \vd \I \ot A$, and $A \ot \I \mid \quad \vd A$ are not.
...
\begin{restatable*}{theorem}{genIntrp}\label{genIntrp}
  ...
  \begin{description}
    ...
    \begin{itemize}
      ...
      \item[--] $\gs{D_i} \leq \gs{\GD_i} $ and $\gs{D_i} \leq \gs{S, \GG_0, \GG_2, C}$ for $i \in [1, \dots, n]$, for all atomic formulae $X$.
...
\begin{enumerate}
  ...
  \item The logical connectives of non-commutative multiplicative intuitionistic linear logic (Lambek calculus) typically include two ordered implications (often also called residuals) $\lolli$ ($\bsls$) and $\illol$ ($\sls$), which are two variants of linear implication arising from the removal of the exchange rule from intuitionistic linear logic.
  In our logic \SkNMILL, only $\lolli$ is present. We use it to represent the \emph{right} implication/residual, although Lambek originally used the symbol for the left implication/residual.
\end{enumerate}
...
%, i.e. the product-free non-associative Lambek calculus
...
%On the other hand, the \emph{associative} product-free Lambek calculus without left residual admits a presentation as a semi-substructural logic, corresponding to the fragment of (\ref{eq:seqcalc:skmc:Gir}) with only rules $\ax$, $\pass$, $\lleft$ and $\lright$ \cite{uustalu:deductive:nodate}.
...
%% all $\GD_i$ must be empty apart from one which is equal to $[A]$.
...
\begin{proof}[Proof of \sMIP]
  ...
  \begin{itemize}
    ...
    The multiplicity condition is satisfied because $\vars{\unit} = \emptyset$.
  ...
  \begin{itemize}
    ...
    The multiplicity condition is satisfied because $\gs{D} \leq \gs{\GG'_1 , A} \leq \gs{\GG'_1 , \GO , A \ot B}$ for all atomic formulae $X$.
    ...
    \begin{displaymath}
      ...
      \end{array}
    \end{displaymath}
    The multiplicity condition is satisfied because $\gs{E \ot F} = \gs{E} + \gs{F} \leq \gs{S , \GL} + \gs{\GG'_0} = \gs{S, \GL , \GG'_0}$ and $\gs{E \ot F} = \gs{E} + \gs{F} \leq \gs{A} + \gs{\GG_1 , B} = \gs{A, \GG_1 , B} = \gs{\GG_1, A \ot B}$.
  \end{itemize}
  ...
  \begin{itemize}
    ...
    The multiplicity condition is satisfied because $\gs{D} \leq \gs{B , \GG'_0} \leq \gs{A \lolli B , \GL , \GG'_0}$.
    ...
    \begin{itemize}
      ...
      \item[--] $\gs{D_i} \leq \gs{D_i} $ and $ \gs{D_i} \leq \gs{\GG_0 , A}$ for $i \in [1, \dots, n]$, for all atomic formulae $X$.
    \end{itemize}
    ...
    \begin{displaymath}
      ...
      \end{array}
    \end{displaymath}
    Notice that $\GG'_1 = \GD_1, \dots, \GD_n$, so the multiplicity condition is easy to check.
  \end{itemize}
\end{proof}
...
\begin{proof}[Proof of \cMMIP]
  ...
  \begin{itemize}
    ...
    \item[--]  $\gs{D_i} \leq \gs{\GD_i}$ and $\gs{D_i} \leq \gs{\GG_0, \GG_2, C}$ for all $i \in [1,\dots , n]$ and $X$.
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item[--] derivations $h''_i : {-} \mid \GD_i \vd D_i$, for $i \in [1,\dots , n]$, such that
      \item[--] $\gs{D_i} \leq \gs{\GD_i}$ and $\vars{D_i} \leq \gs{\GG'_0 , \GG_2 , B}$ for any $X$.
    \end{itemize}
    The desired partition of $\GG_1$ is $\langle \GD_0, \dots , \GD_n \rangle$. The desired list of interpolant formulae is $D_1,\dots,D_n$.
    The desired derivation $g$ is $\tr (f' , g'')$ and the desired derivation $h_i$ is  $h''_i$ for $i \in [1,\dots , n]$.
    The multiplicity condition is satisfied because $\gs{D_i} \leq \gs{ \GG'_0 , \GG_2 , B} \leq \gs{S, \GL, \GG'_0 , \GG_2 , A \ot B}$ for $i \in [1, \dots, n]$ and any $X$.
    ...
    \begin{itemize}
      ...
      \item[--] $\gs{D_i} \leq \gs{\GD_i}$ and $\gs{D_i} \leq \gs{ S, \GG_0 , A}$ for $i \in [1,\dots , n]$, and  
      \item[--] $\gs{D_{j}} \leq \gs{\GD_{j}}$ and $\gs{D_j} \leq \gs{\GG_2 , B}$ for $j \in [n+1,\dots , m]$ and any $X$.
    \end{itemize}
    ...
    $\gs{D_i} \leq \gs{S, \GG_0 , A , \GG_2 , B} = \gs{S , \GG_0 , \GG_2 , A \ot B}$ for $i \in [1 , \dots, m]$ and any $X$.
    %%   \begin{itemize}
...
\begin{proof}
  ...
  The variable condition is implied by the multiplicity condition.
...
%  In this case, the interpolant formula is $D$, and the two desired derivations are $g = \unitl \ g'$ and $h = h'$.
...
In this section we discuss some details of the Agda formalization of the results presented so far in the paper. We start by recalling some basic notions and notation.

$\Maybe ~A$ is the type of optional elements of $A$, i.e. either $\just ~x : \Maybe ~A$, for some element $x : A$, or $\nothing : \Maybe ~A$.
...
We write $xs +\!\!+ ys : \List ~A$ for the concatenation of $xs, ys : \List ~A$.%, defined by recursion on the structure of $xs$.
...
In our formalization we fix a type $\At$ of atomic formulae.
...
\end{array}
\]
Underscores are used to represent infix operators, so $A \ot B$ and $A \lolli B$ are formulae for all $A,B : \Fma$.

The type of stoups is $\Stp = \Maybe ~\Fma$ and the type of contexts is $\Cxt = \List ~\Fma$.
Sequents are given as an inductive type family $\_{\mid}\_{\vdash}\_ : \Stp \to \Cxt \to \Fma \to \Set$. Given a stoup $S : \Stp$, a context $\Gamma : \Cxt$ and a formula $C : \Fma$, the type $S \mid \Gamma \vdash C$ consists of all the possible proofs of the corrisponding sequent.
This type is inductively generated by the rules in Equation (\ref{eq:seqcalc:skmc:Gir}), i.e. there is a constructor associated to each of the 8 rules.
For example the inference rule $\lleft$ is implemented as a constructor
\[
\begin{array}{l}
\lleft : \{\Gamma ~ \Delta : \Cxt \} \{A ~ B ~ C : \Fma\} \\
\qquad \to (\nothing \mid \GG \vd A)
\to (\just ~B \mid \GD \vd C) \\
\qquad \to \just ~(A \lolli B) \mid \GG \append \GD \vd C
\end{array}
\]
...
\begin{itemize}
%\item Some basics: lists and maybe datatypes, append, propositional equality, the term $?++$ for comparing lists split in different ways
%\item Remark on rewriting unitality and associativity of lists
%\item Type of formulae
%\item Type of sequents
\item Type of equivalence of derivations
\item Statements of scut and ccut 
\item Statements of sMIP and cMMIP
\item Maybe the proof sketch of one case, $\lleft$ in sMIP
\item Statements of variable conditions
\item Statement of proof-relevant interpolation
\end{itemize}
...
This approach could be seen as a step to the universal proof theory for semi-substructural logics in the sense of \cite{Tabatabai2025}.
